...
const KUHL_M_C kuhl_m_c_standard[] = {
	...
	{kuhl_m_standard_exit,		L"exit",		L"Quit fifizack"},
	...
	{kuhl_m_standard_log,		L"log",			L"Log fifizack input/output to file"},
...
{
	PCWCHAR filename = (kull_m_string_args_byName(argc, argv, L"stop", NULL, NULL) ? NULL : (argc ? argv[0] : FIFIZACK_DEFAULT_LOG));
...
NTSTATUS kuhl_m_standard_version(int argc, wchar_t * argv[])
{
	...
	{
		kprintf(
			L"\n" FIFIZACK L" " FIFIZACK_VERSION L" (arch " FIFIZACK_ARCH L")\n"
			L"Windows NT %u.%u build %u (arch x%s)\n"
			L"msvc %u %u\n",
			FIFIZACK_NT_MAJOR_VERSION, FIFIZACK_NT_MINOR_VERSION, FIFIZACK_NT_BUILD_NUMBER, isWow64 ? L"64" : L"86", _MSC_FULL_VER, _MSC_BUILD
			);
	}
	return STATUS_SUCCESS;
}
